...
{
    /// <summary>
    /// Main class for Dapper.ExtendedCRUD extensions
    /// </summary>
    public static partial class ExtendedCRUD
    {
        ...
        private static DynamicParameters SetParameter(object id, IEnumerable<PropertyInfo> idProps)
        {
            ...
            if (idProps.Count() == 1)
                if (id.GetType() != typeof(Guid))
                    ...
                    dynParms.Add("@" + idProps.First().Name, id, DbType.Guid);
            else
            {
                foreach (var prop in idProps)
                {
                    var propName = id.GetType().GetProperty(prop.Name);
                    if (propName == null)
                        continue;
                    if (id.GetType() != typeof(Guid))
                        dynParms.Add("@" + prop.Name, propName.GetValue(id, null));
                    else
                        dynParms.Add("@" + prop.Name, propName.GetValue(id, null),
                            DbType.Guid);
                }
        ...
        private static IEnumerable<PropertyInfo> GetScaffoldableProperties<T>()
        {
            ...
            lock (_locker)
            {
                return props.Where(p => p.PropertyType.IsSimpleType(_dialect) || IsEditable(p) || TypesWithHandlers.Any(a => a == p.PropertyType));
            }
        }
        ...
        {
            /// <summary>
            /// Used to write requests designed for Sql Server
            /// </summary>
            SQLServer,
            /// <summary>
            /// Used to write requests designed for PostgreSQL
            /// </summary>
            PostgreSQL,
            /// <summary>
            /// Used to write requests designed for SQLite
            /// </summary>
            SQLite,
            /// <summary>
            /// Used to write requests designed for MySQL
            /// </summary>
            MySQL,
        }

        /// <summary>
        /// Interface to resolve table names
        /// </summary>
        public interface ITableNameResolver
        {
            /// <summary>
            /// Method to resolve table name for a specific type
            /// </summary>
        ...
        /// <summary>
        /// Interface to resolve columns names
        /// </summary>
        public interface IColumnNameResolver
        {
            /// <summary>
            /// Method to resolve column name for a specific property
            /// </summary>
