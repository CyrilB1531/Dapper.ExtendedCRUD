...
namespace Dapper
{
    ...
    /// </summary>
    public static partial class ExtendedCRUD
    {
        /// <summary>
        ...
        public static async Task<T> GetAsync<T>(this IDbConnection connection, object id, IDbTransaction transaction = null, int? commandTimeout = null, CancellationToken cancellationToken = default)
        {
            ...
            var dynParms = SetParameter(id, idProps);

            if (Debugger.IsAttached)
                Trace.WriteLine($"Get<{currenttype}>: {sb} with Id: {id}");

            return await connection.QueryFirstOrDefaultAsync<T>(new CommandDefinition(sb.ToString(), dynParms, transaction, commandTimeout, null, CommandFlags.Buffered, cancellationToken));
        }
        ...
        public static Task<int> UpdateAsync<TEntity>(this IDbConnection connection, TEntity entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null, CancellationToken cancellationToken = default)
        {
            ...
            return connection.ExecuteAsync(new CommandDefinition(sb.ToString(), entityToUpdate, transaction, commandTimeout, cancellationToken: cancellationToken));
        }
        ...
        public static Task<int> DeleteListAsync<T>(this IDbConnection connection, string conditions, object parameters = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            var sb = new StringBuilder();
            sb.AppendFormat("DELETE FROM {0} {1}", name, conditions);
        ...
        public static Task<int> RecordCountAsync<T>(this IDbConnection connection, string conditions = "", object parameters = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            var name = GetTableName(currenttype);
            var sb = new StringBuilder();
            sb.AppendFormat("SELECT COUNT(1) FROM {0} {1}", name, conditions);
        ...
        public static Task<int> RecordCountAsync<T>(this IDbConnection connection, object whereConditions, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            var sb = new StringBuilder();
            var whereprops = GetAllProperties(whereConditions).ToArray();
            sb.AppendFormat("SELECT COUNT(1) FROM {0}", name);
            if (whereprops.Any())
            {
                sb.Append(" where ");
                BuildWhere<T>(sb, whereprops);
